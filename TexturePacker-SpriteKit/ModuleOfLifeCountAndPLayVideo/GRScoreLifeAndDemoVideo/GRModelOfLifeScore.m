...
+ (NSMutableArray *)arryOfNodesWithSize:(CGSize)size {
    ...
    for (int i=0; i<3 ; i++) { //life
        ...
        GRNodeLife *nodeLife = [[GRNodeLife alloc]initWithPosition:CGPointMake(x, size.height - 120)];
        [nodes addObject:nodeLife];
        x = x + 60;
    }

    nodeScore = [[GRNodeScore alloc]initWithPosition:CGPointMake(size.width - 100, size.height - 120) withLabelFont:@"Arial"];
    nodeScore.text = @"0";
